(function initAppMarketEstimator(global) {
  ...
  function sanitizeInput(rawInput) {
    ...
    return {
      ...
      ownReviews30d: Math.max(0, round(num(source.ownReviews30d, 0), 0)),
  ...
  function compute(rawInput, rawContext) {
    ...
    const p25Unit = num(primaryKeyword.p25UnitGross, 0);
    const p50Unit = num(primaryKeyword.p50UnitGross, 0);
    const p75Unit = num(primaryKeyword.p75UnitGross, 0);
    const p90Unit = num(primaryKeyword.p90UnitGross, 0);
    ...
    const targetMarginPct =
      input.allowances.targetMarginPct === null
        ? context.defaultTargetMarginPct
        : input.allowances.targetMarginPct;

    const totalPct = context.referralPct + returnsPct + ppcPct + overheadPct + targetMarginPct;
    const denominator = 1 - totalPct / 100;
    const costBasePerPack = context.landedUnit + context.fbaFeeUnit;
    const mvpPrice = denominator > 0 ? costBasePerPack / denominator : Number.POSITIVE_INFINITY;

    if (!(denominator > 0)) {
      flags.add("NOT_LAUNCHABLE_MVP_ABOVE_CAP");
    }
    ...
    const startPrice = clampPrice(basisPrice, mvpPrice, highPriceCap);
    const minPrice = mvpPrice;
    const maxPrice = highPriceCap;

    if (Number.isFinite(mvpPrice) && Number.isFinite(highPriceCap) && mvpPrice > highPriceCap) {
      flags.add("NOT_LAUNCHABLE_MVP_ABOVE_CAP");
    }

    const reviewGapRaw = num(primaryKeyword.medianReviewsTop10, 0) / (Math.max(0, num(input.ownReviews30d, 0)) + 1);
